function sjekkOmFerdig (){
  let tilSjekk:ruteID[] = ["X7Y7"];
  let erSjekka:ruteID[] = [];
  let denne:ruteID = "X7Y7";
  let denneNr = 0;
  sjekkHer()
 function sjekkHer (){
  //finn ruta med lågast totNR og sett til denne
  if(tilSjekk.length > 0){
    let tmp:number = ruter[tilSjekk[0]].totNR;
...
function finnReiseLengde(nr:ruteID, forelder:ruteID):number {
  let svar = Math.abs(ruter[forelder].reiseLengde + 1);
return Math.floor(svar);
}
function finnLuftjinjeFram(nr:ruteID):number{ 
...
function skipTur(){
const maksFlytt = 2;
let flyttNR:number = maksFlytt;

let flytting = setInterval(flyttSkip,100);
let denneRute:ruteID;
let nesteRute:ruteID = 'X0Y0';
let flyttX = 0;
let flyttY = 0;
  function flyttSkip(){
    if(flyttNR === maksFlytt){
      flyttNR = 1;
      denneRute = nesteRute;
      nesteRute = ruter[denneRute].forelderRute as ruteID;
