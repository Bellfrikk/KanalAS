...
function trykkPaRute(nr:string){
  ...
  if(ruter[nr].type === 'fjell'){ return;
    ...
    if(naboTypar.includes('vatn')){ fyllKanalar(nr)}
    sjekkOmFerdig(nr)
  }
}
...
function fyllKanalar(startRute:string){
  ...
  while(ruterTilSjekk.length > 0){
    ...
    if(ruter[nr].type === 'kanal'){
      ...
      naboer.forEach(nabo => {
        if(!sjekkaRuter.includes(nabo) && ruter[nabo].type === 'kanal'){ ruterTilSjekk.push(nabo);}
      });
       }
    sjekkaRuter.push(nr);
    ruterTilSjekk.shift()
...
function visInfomelding(melding:infoMelding){
